...
import { TrophyIcon, CrownIcon, ArrowUpIcon, RefreshIcon, DashboardIcon } from '../layout/Icons';
      ...
      <div className="relative w-full max-w-xl bg-[#0f0f1a] border border-white/10 rounded-[3rem] p-10 shadow-2xl overflow-hidden animate-in zoom-in slide-in-from-bottom-20 duration-1000">
        ...
        <div className="text-center mb-12">
          <div className="relative inline-block mb-6">
            <div className={`
              w-24 h-24 rounded-3xl flex items-center justify-center text-5xl shadow-2xl border-2 animate-bounce
            ...
            {isWinner && (
              <div className="absolute -inset-4 bg-yellow-400/20 blur-3xl rounded-full animate-pulse -z-10" />
            )}
          </div>
              ...
              >
                <div className="flex items-center gap-4">
                  <div className={`
                    ...
                    ${index === 0 ? 'bg-yellow-400 text-black' : index === 1 ? 'bg-slate-300 text-black' : index === 2 ? 'bg-orange-600 text-white' : 'text-white/20'}
                  `}>
                    {index + 1}
                  </div>
                  <div>
                    ...
                    <div className="text-[10px] text-white/30 uppercase font-black tracking-widest">
                      {player.bankrupt ? 'Eliminated' : 'Finalist'}
                    </div>
                  </div>
                </div>

                <div className="text-right">
                  <div className={`text-sm font-black tabular-nums ${player.bankrupt ? 'text-red-400/50 line-through' : 'text-yellow-400'}`}>
                    {player.balance.toLocaleString()}
                  </div>
                  <div className="text-[9px] text-white/20 uppercase font-black tracking-tighter">RWF Balance</div>
                </div>
              </div>
            );
          })}
        ...
        <div className="flex flex-col gap-3">
          <button
            ...
            className="w-full flex items-center justify-center gap-3 py-5 bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 text-white text-xs font-black uppercase tracking-[0.2em] rounded-3xl transition-all shadow-xl shadow-purple-500/20 active:scale-95 group"
          >
