      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '80px 1.5rem 4rem' }}>
          ...
          <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'flex-start', flexWrap: 'wrap' }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              {stats?.currentWinStreak >= 3 && (
                <div style={{ position: 'absolute', top: -6, right: -6, background: 'var(--gold)', borderRadius: '50%', width: 24, height: 24, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.75rem', fontWeight: 800 }}>
                  🔥
                </div>
              )}
            </div>
            ...
            <div style={{ flex: 1 }}>
              {editing ? (
                  ...
                  <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                    {AVATARS.map(a => (
                      <button key={a} onClick={() => setForm(f => ({ ...f, avatar: a }))} style={{
                        ...
                        <div className={`avatar avatar-${a}`} style={{ width: '100%', height: '100%', borderRadius: '50%', fontSize: '0.65rem' }}>{form.avatar === a ? '✓' : ''}</div>
                      </button>
                    ))}
                  </div>
                ...
                <>
                  ...
                  <button onClick={() => setEditing(true)} className="btn-secondary" style={{ fontSize: '0.82rem', padding: '6px 14px' }}>✏ Edit Profile</button>
                </>
              )}
            </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '1rem', marginBottom: '1.5rem' }}>
          {[
            ...
            { label: 'Wins', value: stats?.gamesWon ?? '—', icon: '🏆', color: 'var(--gold)' },
            { label: 'Win Rate', value: stats?.gamesPlayed ? `${winRate}%` : '—', icon: '📊', color: 'var(--purple-light)' },
            { label: 'Best Streak', value: stats?.longestWinStreak ?? '—', icon: '🔥', color: 'var(--gold)' },
          ].map(s => (
        ...
        <div className="card" style={{ padding: '1.25rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div>
              ...
              <div style={{ fontWeight: 600, color: 'var(--green-pos)' }}>✓ Active</div>
            </div>
            <div>
