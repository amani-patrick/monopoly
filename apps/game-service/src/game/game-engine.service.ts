...
export class GameEngineService {
  ...
  async initGame(roomId: string, players: Omit<Player, 'properties'>[], settings: GameState['settings']): Promise<GameState> {
    ...
    await this.gameRepo.save({
      ...
      roomId,
      status: GameStatus.ACTIVE,
      playerIds: gamePlayers.map(p => p.userId),
      settings,
      startedAt: new Date(),
    });
  ...
  async buyProperty(gameId: string, playerId: string): Promise<GameState> {
    ...
    const propState = state.properties.find(p => p.spaceIndex === player.position)!;
  ...
  async finalizeAuction(gameId: string): Promise<GameState> {
    ...
    if (auction.currentBidderId && auction.currentBid >= auction.startPrice) {
      ...
      const propState = state.properties.find(p => p.spaceIndex === auction.spaceIndex)!;
  ...
  async buildHouse(gameId: string, playerId: string, spaceIndex: number): Promise<GameState> {
    ...
    const owned = state.properties.filter(p => BOARD_SPACES[p.spaceIndex]?.group === space.group && p.ownerId === playerId).length;
    if (owned < groupSize) throw new BadRequestException('Must own full color group to build');

    // Must build evenly across group
    const groupProps = state.properties.filter(p => BOARD_SPACES[p.spaceIndex]?.group === space.group && p.ownerId === playerId);
  ...
  private async applyCard(state: GameState, player: Player, card: GameCard): Promise<GameState> {
    switch (card.action as CardAction) {
      ...
      case 'COLLECT_FROM_PLAYERS':
        for (const other of state.players.filter(p => p.id !== player.id && p.status !== PlayerStatus.BANKRUPT)) {
      ...
      case 'STREET_REPAIRS':
        let total = 0;
        for (const propState of state.properties.filter(p => p.ownerId === player.id)) {
  ...
  private async executeTrade(state: GameState, trade: Trade): Promise<GameState> {
    ...
    for (const spaceIdx of trade.offer.properties) {
      const prop = state.properties.find(p => p.spaceIndex === spaceIdx)!;
      prop.ownerId = to.id;
      from.properties = from.properties.filter(p => p !== spaceIdx);
    ...
    for (const spaceIdx of trade.request.properties) {
      ...
      to.properties = to.properties.filter(p => p !== spaceIdx);
  ...
  async endTurn(gameId: string, playerId: string): Promise<GameState> {
    ...
    const activePlayers = state.players.filter(p => p.status !== PlayerStatus.BANKRUPT);
    ...
    const firstActiveIdx = state.players.findIndex(p => p.status !== PlayerStatus.BANKRUPT);
  ...
  private async checkBankruptcy(state: GameState, player: Player, creditorId?: string): Promise<GameState> {
    ...
    const raisable = state.properties
      .filter(p => p.ownerId === player.id && !p.mortgaged && !p.hotel && p.houses === 0)
      .reduce((sum, p) => sum + (BOARD_SPACES[p.spaceIndex]?.mortgage ?? 0), 0);
  ...
  private async finishGame(state: GameState, winnerId: string): Promise<GameState> {
    ...
    const bankruptPlayers = state.players
      .filter(p => p.status === PlayerStatus.BANKRUPT && p.bankruptAt)
      .sort((a, b) => new Date(b.bankruptAt!).getTime() - new Date(a.bankruptAt!).getTime()); // most recently bankrupt = 2nd place

    bankruptPlayers.forEach((p, i) => { p.rank = i + 2; });
